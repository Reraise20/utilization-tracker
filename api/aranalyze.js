...
async function callKimchi(system, message, model) {
  ...
  return stripReasoningTrace(text);
}

// Strip <think>/<thinking>/<reasoning> blocks that some reasoning models (e.g. minimax-m2.7,
// nemotron-3-super) emit before the final answer. We strip server-side so the trace never
// leaks into the UI, exports, or downstream processing.
function stripReasoningTrace(text) {
  ...
  // Unclosed <think> at the start — strip up to first markdown heading
  const trimmed = cleaned.trim();
  if (/^<think/i.test(trimmed) || /^<thinking/i.test(trimmed)) {
    const m = cleaned.match(/\n#{1,3}\s+/);
    if (m && m.index !== undefined) cleaned = cleaned.slice(m.index);
  }
  return cleaned.trim();
}
