...
export default async function handler(req, res) {
  ...
  try {
    const { system, message, preferProvider } = req.body;

    // ─── Route based on preferProvider parameter ──────────────────────────
    // If preferProvider === "anthropic", Claude is primary, Gemini is fallback.
    // Otherwise (default), Gemini is primary, Claude is fallback.

    if (preferProvider === "anthropic") {
      // 1️⃣ Try Claude first
      try {
        const text = await callAnthropic(system, message);
        return res.status(200).json({ text, provider: "anthropic" });
      } catch (anthropicError) {
        console.warn("[aranalyze] Anthropic failed, falling back to Gemini:", anthropicError.message);
      }

      // 2️⃣ Fallback to Gemini
      const text = await callGemini(system, message);
      return res.status(200).json({ text, provider: "gemini-fallback" });
    }

    // ─── Default routing: Gemini primary, Claude fallback ─────────────────
    try {
      const text = await callGemini(system, message);
