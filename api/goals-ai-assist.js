...
//
// Uses minimax-m2.7 via Kimchi (CAST AI) — same pattern as api/aranalyze.js.
// No npm dependencies. Reuses KIMCHI_API_KEY env var already set in Vercel.
//
// Usage:
...
const KIMCHI_MODEL = "minimax-m2.7";
const KIMCHI_URL   = "https://llm.kimchi.dev/openai/v1/chat/completions";


// ── Kimchi (OpenAI-compatible) ──────────────────────────────────────────────
async function callKimchi(system, message) {
  const KIMCHI_KEY = process.env.KIMCHI_API_KEY;
  if (!KIMCHI_KEY) throw new Error("KIMCHI_API_KEY not set in Vercel env");
  ...
  const r = await fetch(KIMCHI_URL, {
    method: "POST",
    headers: {
  ...
  if (!r.ok) {
    ...
    throw new Error("Kimchi " + r.status + ": " + txt.slice(0, 300));
  }
  return r.json();
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await callKimchi(SYSTEM_PROMPT, userMessage);
