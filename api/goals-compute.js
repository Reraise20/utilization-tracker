...
// ── Task config — MUST match TASKS array in index.html (task_type = id) ─────
// mode 'count' → daily target divides the count; mode 'hours' → target divides hours.
// 'downtime' is intentionally excluded (non-productive, not counted toward goal).
const TASKS = [
  { id: "mcs",      mode: "count", target: 30 },
  { id: "audit",    mode: "count", target: 30 },
  { id: "payer",    mode: "hours", target: 8 },
  { id: "jira",     mode: "hours", target: 8 },
  { id: "research", mode: "hours", target: 8 },
  { id: "others",   mode: "hours", target: 8 },
];
...
function computeUtilization(def, ctx) {
  ...
  // EXACT replication of getRangeStats() / avgUtil in the tracker dashboard:
  //   1. Sum counts/hours across the whole period (grand total per task type)
  //   2. totalUtil = ((mcs/30)+(audit/30)+((payer+jira+research+others+downtime)/8))*100
  //   3. avgUtil   = totalUtil / (number of distinct days with entries)
  // NOTE: downtime IS included here, matching the dashboard exactly so the two agree.
  const byType = { mcs: 0, audit: 0, payer: 0, jira: 0, research: 0, others: 0, downtime: 0 };
  for (const e of myEntries) {
    const tt = e.task_type;
    if (tt === "mcs" || tt === "audit") {
      byType[tt] += (e.count || 0);
    } else if (byType[tt] != null) {
      byType[tt] += (parseFloat(e.hours) || 0);
    }
  }

  const distinctDays = new Set(myEntries.map(e => e.date)).size;
  ...
  const avgUtil = distinctDays > 0 ? totalUtil / distinctDays : 0;

  // Projection: same average assumed to hold (average-of-period model).
  const projectedPct = avgUtil;

  return scoredRow(def,
    round2(avgUtil), applyRatingBand(def, avgUtil),
    `${avgUtil.toFixed(1)}% avg daily utilization over ${distinctDays} day${distinctDays === 1 ? "" : "s"} with entries`,
    'auto',
...
function computeMCSPastDue(def, ctx) {
  ...
  // Default assumption: 0% past due (Pass) unless a day was logged with past-due inventory.
  if (!vals.length) {
    return scoredRow(def, 0, def.rating_bands.pass.score,
      'No past-due logged this month — assumed 0% (clear)', 'auto');
  }
  const worst  = Math.max(...vals);
  const passed = worst <= 0;
  const rating = passed ? def.rating_bands.pass.score : def.rating_bands.fail.score;
  return scoredRow(def, round2(worst), rating,
    `Worst day: ${worst.toFixed(2)}% past due (over ${vals.length} daily entr${vals.length === 1 ? "y" : "ies"})`, 'manual');
}

function computeJiraTAT(def, ctx) {
  const vals = ctx.dailyInputs.map(d => d.jira_out_of_tat_count).filter(v => v != null);
  // Default assumption: 0 instances out of TAT unless a day logged otherwise.
  if (!vals.length) {
    return scoredRow(def, 0, applyRatingBand(def, 0),
      'No out-of-TAT logged this month — assumed 0 instances', 'auto');
  }
  const worst = Math.max(...vals);
