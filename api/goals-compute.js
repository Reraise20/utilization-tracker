...
function computeUtilization(def, ctx) {
  ...
  // Per-day utilization %, exactly like calcUtil() in the tracker:
  //   for each task type, count/target*100 (count mode) or hours/target*100 (hours mode)
  //   sum across task types = that day's utilization %.
  // Monthly utilization = average across days that have any entry.
  const byDate = {};
  ...
  for (const date of Object.keys(byDate)) {
    ...
    for (const t of TASKS) {
      const rows = dayEntries.filter(e => e.task_type === t.id);
      if (t.mode === "count") {
        u += (rows.reduce((s, e) => s + (e.count || 0), 0) / t.target) * 100;
      } else {
        u += (rows.reduce((s, e) => s + (parseFloat(e.hours) || 0), 0) / t.target) * 100;
      }
    }
    dailyUtils.push(u);
  }

  const daysWithEntries = dailyUtils.length;
  const avgUtil = daysWithEntries > 0
    ? dailyUtils.reduce((s, v) => s + v, 0) / daysWithEntries
    : 0;

  // Projection: assume the same average holds for the rest of the month.
  // (Average-of-days model means the projected % equals current average.)
  const projectedPct = avgUtil;

  return scoredRow(def,
    round2(avgUtil), applyRatingBand(def, avgUtil),
    `${avgUtil.toFixed(1)}% avg daily utilization over ${daysWithEntries} day${daysWithEntries === 1 ? "" : "s"} with entries`,
    'auto',
